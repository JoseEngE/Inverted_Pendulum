...
void pwm_init(void) {
  ...
  gpio_config_t dir_gpio_config = {
      .pin_bit_mask =
          (1ULL << LEDC_DIRECTION_IO), // <-- Usa la definición del .h (GPIO 33)
  ...
  // ESP_LOGI(TAG, "LEDC configurado en GPIO %d a %d Hz", LEDC_OUTPUT_IO,
  // LEDC_FREQUENCY); ESP_LOGI(TAG, "LEDC configurado: Pin Pulsos GPIO %d, Pin
  // Dir GPIO %d, a %d Hz", LEDC_OUTPUT_IO, LEDC_DIRECTION_IO, LEDC_FREQUENCY);
...
int execute_movement(int num_pulses, int frequency, int direction) {
  // ESP_LOGI(TAG, "Ejecutando movimiento: %d pulsos a %d Hz, Dir: %d",
  // num_pulses, frequency, direction);
  if (num_pulses <= 0) {
  ...
  // ESP_LOGI(TAG, "Pin de dirección (GPIO %d) puesto a %d", LEDC_DIRECTION_IO,
  // direction);
  ...
  /*if (freq_err != ESP_OK) {
      ESP_LOGE(TAG, "Error al establecer la frecuencia a %d Hz: %s", frequency,
...
void motor_control_task(void *arg) {
  ...
  while (1) {
    // Espera de forma bloqueante hasta recibir la última velocidad deseada del
    // PID
    if (xQueueReceive(motor_command_queue, &received_cmd, portMAX_DELAY) ==
        pdTRUE) {
      // La odometría ahora se maneja dentro del bucle del PID
      // (pid_controller_task)

      // Si frequency es 0, significa una instrucción para detener el motor
      if (received_cmd.frequency == 0) {
        // Detener la generación de pulsos apagando el duty cycle
        ESP_ERROR_CHECK(ledc_set_duty(LEDC_MODE, LEDC_CHANNEL, 0));
        ESP_ERROR_CHECK(ledc_update_duty(LEDC_MODE, LEDC_CHANNEL));
        continue; // Volver a esperar el siguiente comando
      }
      ...
      // 2. Ajustar la frecuencia del PWM dinámicamente
      ESP_ERROR_CHECK(
          ledc_set_freq(LEDC_MODE, LEDC_TIMER, received_cmd.frequency));

      // 3. Mantener el pulso activo con un duty del 50%
      int duty_cycle = (1 << LEDC_DUTY_RES) / 2;
      ESP_ERROR_CHECK(ledc_set_duty(LEDC_MODE, LEDC_CHANNEL, duty_cycle));
      ESP_ERROR_CHECK(ledc_update_duty(LEDC_MODE, LEDC_CHANNEL));

      // OBSERVACIÓN: YA NO HAY vTaskDelay AQUÍ.
      // El motor seguirá operando a la frecuencia establecida de manera
      // asincrónica e ininterrumpida hasta que llegue el siguiente paquete del
      // PID re-ajustando la frecuencia o parando a 0.
    }
  }
}
